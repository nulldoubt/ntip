...
for command in ip sysctl nft tc conntrack iperf3 ping jq timeout install getent \
    stat id awk grep tail sleep chown chmod mkdir tr python3
do
...
if ! getent passwd ntip >/dev/null 2>&1 || ! getent group ntip-admin >/dev/null 2>&1; then
    echo "the ntip service user and ntip-admin group are required" >&2
...
install -m 0644 "$repo_root/packaging/config/server.json" "$work/server.json"
...
"$ntsrv" $server_args node create node01 --vnr vnr0 --addr 10.1.0.2 \
    --credential-out "$work/node01.enrollment"
# shellcheck disable=SC2086
"$ntsrv" $server_args node create node02 --vnr vnr0 --addr 10.1.0.3 \
    --credential-out "$work/node02.enrollment"
# shellcheck disable=SC2086
"$ntsrv" $server_args route add 192.168.178.0/24 node01

# Configuration stores credentials but does not connect.
# shellcheck disable=SC2086
"$ntcl" $node1_args config 198.18.0.1:49152 node01 \
    --credential-file "$work/node01.enrollment"
# shellcheck disable=SC2086
"$ntcl" $node2_args config 198.18.1.1:49152 node02 \
    --credential-file "$work/node02.enrollment"
